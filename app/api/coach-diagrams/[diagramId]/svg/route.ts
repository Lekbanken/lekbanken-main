/**
 * GET /api/coach-diagrams/[diagramId]/svg
 *
 * Serve SVG for a coach diagram.
 * Mixed auth: authenticated user → always allowed, participant → must verify
 * game references diagram via game_steps.media_ref → game_media.media_id chain.
 *
...
export const GET = apiHandler({
  ...
  handler: async ({ req, params }) => {
    ...
    if (!user) {
      ...
      if (!participant || REJECTED_PARTICIPANT_STATUSES.has(participant.status ?? '')) {
        return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
      }
