...
export function ParticipantRow({
  ...
  const [confirmAction, setConfirmAction] = useState<'kick' | 'block' | null>(null);

  const closeDialogs = () => setConfirmAction(null);
  ...
  return (
    <>
            ...
            {isMenuOpen && (
              <>
                ...
                <div
                  role="menu"
                  aria-label={t('participantActions')}
                  className="absolute right-0 top-full z-20 mt-1 min-w-[180px] rounded-lg border border-border bg-card p-1 shadow-lg"
                >
                  {participant.status === 'idle' && onApprove && (
                    <button
                      onClick={handleApprove}
                      role="menuitem"
                      className="flex w-full min-h-11 items-center gap-2 rounded px-3 py-2.5 text-left text-sm hover:bg-muted"
                    >
                      <HandRaisedIcon className="h-4 w-4 text-amber-500" />
                        {t('approve')}
                    </button>
                    ...
                    <>
                      ...
                      {Array.from({ length: Math.min(participantCount, 10) }, (_, i) => i + 1).map((pos) => (
                        <button
                          ...
                          className={cn(
                            'flex w-full min-h-10 items-center gap-2 rounded px-3 py-2 text-left text-sm hover:bg-muted',
                  ...
                  {!isCurrentUser && !isSessionEnded && onKick && (
                    <button
                      onClick={handleKick}
                      role="menuitem"
                      className="flex w-full min-h-11 items-center gap-2 rounded px-3 py-2.5 text-left text-sm hover:bg-muted"
                  ...
                  {!isCurrentUser && onBlock && (
                    <button
                      onClick={handleBlock}
                      role="menuitem"
                      className="flex w-full min-h-11 items-center gap-2 rounded px-3 py-2.5 text-left text-sm text-destructive hover:bg-muted"
                    >
      ...
      <AlertDialog open={confirmAction === 'kick'} onOpenChange={(open) => !open && closeDialogs()}>
        <AlertDialogContent variant="destructive">
          <AlertDialogHeader>
            <AlertDialogTitle>{t('confirmKick.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('confirmKick.description', { name: participant.displayName })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('confirmKick.cancel')}</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={confirmKick}>
              {t('confirmKick.confirm')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

      <AlertDialog open={confirmAction === 'block'} onOpenChange={(open) => !open && closeDialogs()}>
        <AlertDialogContent variant="destructive">
          <AlertDialogHeader>
            <AlertDialogTitle>{t('confirmBlock.title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('confirmBlock.description', { name: participant.displayName })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('confirmBlock.cancel')}</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={confirmBlock}>
