...
- Middleware: `middleware.ts` enforces auth for `/app` and `/admin`, guards guest-only auth routes, applies admin gate via JWT claims, sets `x-request-id`, refreshes signed httpOnly `lb_tenant`, and attaches `x-tenant-id` when known.
