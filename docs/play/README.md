...
> Entry point for the play documentation cluster. This folder mixes active play references, contracts, and dated audit snapshots that must be read together with the root-level play structure guardrails.
...
The root-level play structure audits remain the guardrails for large-scale refactors. The files in this folder are more feature- and UI-specific.
...
If a file in this folder conflicts with the current play structure guardrails or the code, prefer the root-level play structure audits for structural questions and the active contracts in this folder for surface-specific rules.