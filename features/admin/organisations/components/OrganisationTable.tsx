...
type OrganisationTableProps = {
  ...
  isLoading: boolean;
  searchQuery?: string;
  onEdit: (organisation: AdminOrganisationListItem) => void;
  onStatusChange: (organisationId: string, status: OrganisationListStatus) => void;
...
const statusStyles: Record<OrganisationListStatus, { dot: string; text: string }> = {
  active: { dot: "bg-emerald-500", text: "text-emerald-600" },
  inactive: { dot: "bg-muted-foreground/50", text: "text-muted-foreground" },
  trial: { dot: "bg-yellow-500", text: "text-yellow-600" },
...
export function OrganisationTable({
  ...
  const handleRowClick = (event: MouseEvent<HTMLTableRowElement>, organisation: AdminOrganisationListItem) => {
    if ((event.target as HTMLElement).closest("[data-no-row-click]")) return;
    // Navigate to detail page instead of opening edit dialog
      ...
      <table className="min-w-full">
        ...
        <tbody className="divide-y divide-border/50 bg-card">
          {organisations.map((org) => {
            ...
            const planStyle = org.billing?.plan 
              ? planBadgeStyles[org.billing.plan] || planBadgeStyles.default 
              : null;
