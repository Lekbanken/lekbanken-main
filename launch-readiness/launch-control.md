...
| Demo | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | Audit complete — 18 findings (0 P0, 3 P1, 10 P2, 5 P3). Own domain per GPT directive (Tier 1 — externt exponerad). **M1 ✅** — 3 P1 fixed: DEMO-001 (launch-sufficient persistent rate limiter), DEMO-002 (hardcoded access code removed), DEMO-003 (error leak removed). 0 P1 remaining. **Regression ✅ (2026-03-14)** — all 8 GPT-defined areas pass, 3 M1 fixes verified intact, demo flow test-group-ready. **3 new findings:** REG-DEMO-001 (P2, demo-expired form renders raw JSON), REG-DEMO-002 (P2, demo_sessions RLS overly permissive), REG-DEMO-003 (P3, RPC ownership check missing). See `audits/demo-regression-audit.md`. |
