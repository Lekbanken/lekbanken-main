...
| Planner | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | Audit complete + GPT-calibrated — 16 findings (0 P0, 8 P1, 6 P2, 2 P3). 27 routes audited, 0 unwrapped, RLS verified on all 10 tables. **M1 ✅** **M2 ✅** **M3 ✅** — 8 P1 fixed (PLAN-001–008). 0 P1 remaining. **Launch-scope complete.** Regression audit ✅ (2026-03-14) — M1-M3 intact, 5 new routes all wrapped, 2 new findings (1 P2, 1 P3). See `audits/planner-regression-audit.md`. |
...
| Demo | ✅ | ❌ | — | ❌ | ❌ | ❌ | Audit complete — 18 findings (0 P0, 3 P1, 10 P2, 5 P3). Own domain per GPT directive. 8 route files, ephemeral user system, feature gating, conversion tracking, cleanup function. **3 P1 pending:** DEMO-001 (in-memory rate limit), DEMO-002 (hardcoded access code), DEMO-003 (error leak). See `audits/demo-audit.md`. |
