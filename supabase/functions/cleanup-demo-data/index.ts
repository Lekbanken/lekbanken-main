...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 4. Ta bort demo-användare (användare med is_ephemeral = true och ingen aktiv session)
    ...
    // Ta bort alla utgångna demo-användare
    // OBS: "converted" flaggan betyder bara att de klickade på "Kontakta sälj" eller liknande,
    // INTE att de blev riktiga användare. Demo-användare måste logga ut och skapa nytt konto
    // för att köpa licens, så det finns ingen anledning att skydda dem.
    if (orphanedDemoUsers && orphanedDemoUsers.length > 0) {
      ...
      if (userIds.length > 0) {
        ...
        const { data: deletedUsers, error: userDeleteError } = await supabase
          ...
          .delete()
          .in('id', userIds)
          .eq('is_ephemeral', true) // Extra säkerhet
          .select('id');
