-- =============================================================================
-- Migration: 20260202100000_fix_atomic_upsert_null_tenant.sql
-- Purpose: Fix upsert_game_content_v1 to allow games with NULL owner_tenant_id
--
-- BUG: The original function checked "if v_owner_tenant_id is null" to detect
--      missing games, but this also fails for global games (no tenant).
-- FIX: Use a separate EXISTS check to verify the game exists.
-- =============================================================================
...
begin
  ...
  -- ---------------------------------------------------------------------------
  -- FIX: Use EXISTS to check game presence, separate from owner_tenant_id check
  ...
  -- Get owner_tenant_id for tenant mismatch check (can be NULL for global games)
  ...
  -- Only check if both expected and actual are non-null
  ...
  begin
    ...
    then
      ...
      select
        ...
        coalesce(r->>'private_instructions', ''),
        r->>'private_hints',
        coalesce((r->>'min_count')::int, 1),
        nullif(r->>'max_count', '')::int,
        coalesce(r->>'assignment_strategy', 'random'),
        (r->'scaling_rules')::jsonb,
        case when r ? 'conflicts_with' then array(select jsonb_array_elements_text(r->'conflicts_with')) else null end,
        r->>'locale'
    ...
    if (p_payload ? 'materials') and p_payload->'materials' is not null then
      insert into public.game_materials (
        game_id, items, safety_notes, preparation, locale
      )
      values (
        v_game_id,
    ...
    -- ---- Artifact variants (flat array, pre-resolved role IDs) --------------
  ...
  exception when others then
    -- Subtransaction rollback: all changes within this block are undone
    raise log 'upsert_game_content_v1 FAILED run=% game=% error=% code=%',
      coalesce(v_import_run_id::text, 'null'),
      v_game_id,
      sqlerrm,
      sqlstate;
...
-- Maintain same permissions
comment on function public.upsert_game_content_v1(jsonb)
  is 'Atomic upsert (delete+insert) for game content - v1.1 fixed null tenant check';
...
-- Note: authenticated role removed in 20260202000000
