-- =============================================================================
-- Migration: 20260202100002_fix_atomic_upsert_role_conflicts.sql
-- Purpose: Fix conflicts_with column type casting from text[] to uuid[]
--
-- BUG: conflicts_with column is uuid[] but we're inserting text[]
-- FIX: Cast jsonb text array to uuid[] explicitly
-- =============================================================================
...
begin
  ...
  begin
    ...
    -- Roles (fixed: cast text[] to uuid[] for conflicts_with)
    if (p_payload ? 'roles') 
    ...
    then
      ...
      select
        ...
        -- FIXED: cast text array to uuid array
        case 
          when r ? 'conflicts_with' and jsonb_array_length(r->'conflicts_with') > 0 
...
comment on function public.upsert_game_content_v1(jsonb)
  is 'Atomic upsert for game content - v1.3 fixed uuid[] casting for conflicts_with';
