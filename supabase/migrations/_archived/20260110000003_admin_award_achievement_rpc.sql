-- Admin Award Achievement RPC v1
-- Purpose: SECURITY DEFINER function for awarding achievements to users
-- Date: 2026-01-10

BEGIN;

--------------------------------------------------------------------------------
-- 1) ADMIN AWARD ACHIEVEMENT FUNCTION
--------------------------------------------------------------------------------
...
-- Grant execute to authenticated users (function validates system_admin internally)
GRANT EXECUTE ON FUNCTION public.admin_award_achievement_v1(UUID, UUID[], TEXT, UUID, TEXT) TO authenticated;

--------------------------------------------------------------------------------
-- 2) HELPER: GET TENANT USERS FOR BULK AWARD
--------------------------------------------------------------------------------
...
GRANT EXECUTE ON FUNCTION public.get_tenant_user_ids(UUID) TO authenticated;
...
'Award an achievement to one or more users. System admin only. Idempotent via idempotency_key.
...
'Get all active user IDs for a tenant. System admin only. Used for "award to all users in tenant" feature.';
