-- Fix: resolve ambiguous column reference "balance" in apply_coin_transaction_v1
-- The issue is that the UPDATE ... SET balance = balance + v_delta references both
-- the return column "balance" and the table column user_coins.balance.
-- Solution: Use table alias in UPDATE to disambiguate.
