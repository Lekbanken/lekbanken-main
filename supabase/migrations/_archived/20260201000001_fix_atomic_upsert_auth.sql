-- =============================================================================
-- Migration: 20260201000001_fix_atomic_upsert_auth.sql
-- Purpose: Fix service_role detection in upsert_game_content_v1
--
-- Problem: current_user <> 'service_role' doesn't work in PostgREST context.
-- Solution: Check JWT claims for role = 'service_role'
-- =============================================================================
...
begin
  ...
  --   - service_role (JWT claim role = 'service_role')
  -- Note: current_user doesn't work in PostgREST context for service_role detection
  v_jwt_role := coalesce(
...
comment on function public.upsert_game_content_v1(jsonb)
  is 'Atomic game content upsert (v1). All-or-nothing for child content via subtransaction. Auth fix: uses JWT claims for service_role detection.';
